...
class ScorerFeatBasedLSTM(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
            ...
            for phn_index in range(len(phns_ali[utt_index])):
                ...
                if phns_ali[utt_index][phn_index] != 'sil':
                    ...
                    if phn_end_index > x.shape[1]:
                        ...
                        raise Exception("Utterance length exceeded.")
        ...
        x_phn_lens = torch.tensor([len(utt_phn_x) - 1 for utt_x in x_rearranged for utt_phn_x in utt_x])
        ...
        emb_actual_final = torch.concat(emb_actual_rearranged, 0)
        emb_actual_final = self.modules.scorer_nn(emb_actual_final)
        ...
        if self.hparams.similarity_calc == "cosine":
            ...
            scores_pred = torch.nn.functional.cosine_similarity(
                phone_rep_pred, emb_actual_final, dim=len(phone_rep_pred.shape) - 1)
        elif self.hparams.similarity_calc == "euclidean":
            # Normalized Euclidean distance (NED)
            scores_pred = 1 - 0.5 * (phone_rep_pred - emb_actual_final).var(dim=2) / \
                          (phone_rep_pred.var(dim=2) + emb_actual_final.var(dim=2))
        else:
            scores_pred = self.modules.scorer_similarity_nn(torch.concat([phone_rep_pred, emb_actual_final], dim=2)) \
                .view(self.hparams.batch_size, emb_actual_final.shape[1])
